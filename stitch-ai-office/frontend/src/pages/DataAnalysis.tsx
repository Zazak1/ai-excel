...
const DataAnalysis = () => {
  ...
  const summary = useMemo<Summary>(() => {
    ...
    if (numericColumns === 0) notes.push('未检测到数值列；可粘贴包含数值的 CSV 以查看统计。');
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-6">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">本地 CSV 粘贴解析与摘要统计（无第三方图表依赖）。</p>
        </div>
        <div className="flex items-center gap-2">
          <Button
            variant="secondary"
            onClick={() => setCsv('date,revenue,cost\n2025-01-01,120,80\n2025-01-02,140,90\n2025-01-03,160,110\n')}
          >
            恢复示例
          </Button>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <Card>
          <CardHeader className="flex items-center justify-between">
            <p className="text-sm font-black text-slate-900">CSV 输入</p>
            <Badge variant="warning">placeholder</Badge>
          </CardHeader>
          <CardContent>
            <textarea
              className="w-full min-h-[420px] bg-slate-50 border border-slate-200 rounded-xl p-4 text-sm text-slate-800 focus:ring-2 focus:ring-[#1337ec] focus:border-[#1337ec] focus:bg-white transition-colors font-mono"
              value={csv}
